...
{
    public class RealDevengadoDialog : ComponentDialog
    {
        public RealDevengadoDialog() : base(nameof(RealDevengadoDialog))
        {
        ...
        private async Task<DialogTurnResult> ShowUsers(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    var NumCuenta = reader["Pos_Pre"].ToString();
                    var sociedad = society;
                    var saldoPr = Convert.ToDecimal(reader["Deven_Anual_Gastado_Total"]);
