...
namespace Bot.Api.Dialogs
{
    public class ImporteLiberadoDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> ShowUsers(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            // PPTO_Actual_Anual_Importe_Disponible_liberado

            // Construct the query using the CeCo and NumCuenta parameters
            var query = $@"SELECT Desc_CeGe, Centro_Gestor, Desc_PosPre, Pos_Pre, PPTO_Actual_Anual_Importe_Disponible_liberado
                   FROM {tableName}
                   WHERE Centro_Gestor IN ({string.Join(",", ceco.Select(c => $"'{c}'"))})
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    var NumCuenta = reader["Pos_Pre"].ToString();
                    var sociedad = society;
                    var saldoPr = Convert.ToDecimal(reader["PPTO_Actual_Anual_Importe_Disponible_liberado"]);
