...
{
    public class DisponibleAlMomentoDialog : ComponentDialog
    {
        public DisponibleAlMomentoDialog() : base(nameof(DisponibleAlMomentoDialog))
        {
        ...
        {
            await stepContext.BeginDialogAsync(nameof(ReadInfoDialog), cancellationToken: cancellationToken);
        ...
        private async Task<DialogTurnResult> ShowUsers(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            // Importe_Disponible_Anual_Disponible_al_momento

            // Construct the query using the CeCo and NumCuenta parameters
            var query = $@"SELECT Desc_CeGe, Centro_Gestor, Desc_PosPre, Pos_Pre, Importe_Disponible_Anual_Disponible_al_momento
                   FROM {tableName}
                   WHERE Centro_Gestor IN ({string.Join(",", ceco.Select(c => $"'{c}'"))})
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    var NumCuenta = reader["Pos_Pre"].ToString();
                    var sociedad = society;
                    var saldoPr = Convert.ToDecimal(reader["Importe_Disponible_Anual_Disponible_al_momento"]);
